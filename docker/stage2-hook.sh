...
ARGUS_HOME="${ARGUS_HOME:-/opt/data}"
...
# $ARGUS_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass ARGUS_UID/ARGUS_GID — or the PUID/PGID aliases — which the
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e ARGUS_UID=\$(id -u) -e ARGUS_GID=\$(id -g) ...
...
# --- Bootstrap ARGUS_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid hermes mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ARGUS_HOME paths whose parents only
# root can create (e.g. `ARGUS_HOME=/home/hermes/.argus` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ARGUS_UID/ARGUS_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ARGUS_UID/
# ARGUS_GID still win when both are set.  See #15290, salvages #25872.
ARGUS_UID="${ARGUS_UID:-${PUID:-}}"
ARGUS_GID="${ARGUS_GID:-${PGID:-}}"

if [ -n "${ARGUS_UID:-}" ] && validate_uid_gid "$ARGUS_UID" && [ "$ARGUS_UID" != "$(id -u hermes)" ]; then
    echo "[stage2] Changing hermes UID to $ARGUS_UID"
    usermod -u "$ARGUS_UID" hermes
fi
if [ -n "${ARGUS_GID:-}" ] && validate_uid_gid "$ARGUS_GID" && [ "$ARGUS_GID" != "$(id -g hermes)" ]; then
    echo "[stage2] Changing hermes GID to $ARGUS_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ARGUS_GID" hermes 2>/dev/null || true
fi
...
# When ARGUS_UID is remapped or the top-level $ARGUS_HOME isn't owned by
# the runtime hermes UID, restore ownership to hermes — but ONLY for the
# directories hermes actually writes to. The full $ARGUS_HOME may be a
...
needs_chown=false
if [ "$(stat -c %u "$ARGUS_HOME" 2>/dev/null)" != "$actual_hermes_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ARGUS_HOME (targeted) to hermes ($actual_hermes_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ARGUS_HOME: chown the directory itself (not its contents)
    # so hermes can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$ARGUS_HOME"; then
        :
    else
        chown hermes:hermes "$ARGUS_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $ARGUS_HOME failed (rootless container?) — continuing"
    fi
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$ARGUS_HOME/$sub" ]; then
            chown_hermes_tree "$ARGUS_HOME/$sub"
        fi
    done
fi
...
# $ARGUS_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# ARGUS_DISABLE_LAZY_INSTALLS=1. Keeping /opt/hermes root-owned and
...
# $ARGUS_HOME/lazy-packages dir on the data volume (Dockerfile sets
# ARGUS_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# Always reset ownership of $ARGUS_HOME/profiles to hermes on every
...
# chown would fail.
if [ -d "$ARGUS_HOME/profiles" ]; then
    chown_hermes_tree "$ARGUS_HOME/profiles"
fi

# Always reset ownership of $ARGUS_HOME/cron on every boot for the same
...
if [ -d "$ARGUS_HOME/cron" ]; then
    chown_hermes_tree "$ARGUS_HOME/cron"
fi
...
# *subdirectories*; loose state files living directly under $ARGUS_HOME
...
# sweep so host-owned files in a bind-mounted $ARGUS_HOME are never
...
for f in \
    ...
    active_profile; do
    if [ -e "$ARGUS_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$ARGUS_HOME/$f"; then
            :
        else
            chown hermes:hermes "$ARGUS_HOME/$f" 2>/dev/null || true
        fi
    fi
done
...
if [ -f "$ARGUS_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$ARGUS_HOME/config.yaml"; then
        :
    else
        chown hermes:hermes "$ARGUS_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$ARGUS_HOME/config.yaml" 2>/dev/null || true
    fi
fi
...
# shell isn't a second interpreter — defends against $ARGUS_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_hermes mkdir -p \
    "$ARGUS_HOME/cron" \
    "$ARGUS_HOME/sessions" \
    "$ARGUS_HOME/logs" \
    "$ARGUS_HOME/logs/gateways" \
    "$ARGUS_HOME/hooks" \
    "$ARGUS_HOME/memories" \
    "$ARGUS_HOME/skills" \
    "$ARGUS_HOME/skins" \
    "$ARGUS_HOME/plans" \
    "$ARGUS_HOME/workspace" \
    "$ARGUS_HOME/home" \
    "$ARGUS_HOME/pairing" \
    "$ARGUS_HOME/platforms/pairing" \
    "$ARGUS_HOME/lazy-packages"
...
# $ARGUS_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $ARGUS_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.argus:/opt/data) and sometimes shared with a
...
# 'argus update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $ARGUS_HOME if one is present (the host install's
...
if [ -f "$ARGUS_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$ARGUS_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$ARGUS_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ARGUS_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$ARGUS_HOME/$dest"; then
            :
        else
            as_hermes cp "$INSTALL_DIR/$src" "$ARGUS_HOME/$dest"
        fi
    fi
}
...
if [ -f "$ARGUS_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$ARGUS_HOME/.env"; then
        :
    else
        chown hermes:hermes "$ARGUS_HOME/.env" 2>/dev/null || true
        chmod 600 "$ARGUS_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $ARGUS_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `argus update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set ARGUS_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$ARGUS_HOME/config.yaml" ]; then
...
if [ ! -f "$ARGUS_HOME/auth.json" ] && [ -n "${ARGUS_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$ARGUS_HOME/auth.json"; then
        :
    else
        printf '%s' "$ARGUS_AUTH_JSON_BOOTSTRAP" > "$ARGUS_HOME/auth.json"
        chown hermes:hermes "$ARGUS_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$ARGUS_HOME/auth.json"
    fi
fi
...
# can set ARGUS_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# ARGUS_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$ARGUS_HOME/gateway_state.json" ] && \
        [ "${ARGUS_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$ARGUS_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$ARGUS_HOME/gateway_state.json"
        chown hermes:hermes "$ARGUS_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$ARGUS_HOME/gateway_state.json"
    fi
fi
