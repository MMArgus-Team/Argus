...
def cmd_migrate(args: Any) -> int:
    ...
    print("usage: argus migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `argus migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `argus doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
