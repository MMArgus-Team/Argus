...
#
# Many modules cache the Argus home (ARGUS_HOME) at import time (module-level
# constants). We intercept --profile/-p from sys.argv here and set the env var
# so that every subsequent ``os.getenv("ARGUS_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.argus/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ARGUS_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    # 1.5 If ARGUS_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.argus/profiles/coder or
    # /opt/data/profiles/coder).  If ARGUS_HOME points to the Argus root
    # instead (e.g. systemd hardcodes ARGUS_HOME=/root/.argus), we must
...
def _hermes_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    self-replacement hazard exists outside Windows. Enumerates the CURRENT
    project console scripts (the legacy `hermes*` shims were removed).
    """
    if not _is_windows():
        return []
    return [
        scripts_dir / "argus.exe",
        scripts_dir / "argus-agent.exe",
        scripts_dir / "argus-acp.exe",
        scripts_dir / "mm-argus.exe",
    ]
...
def _detect_concurrent_hermes_instances(
    ...
    The Argus Desktop Electron app spawns ``argus.EXE`` as a backend child,
    ...
    shims (``argus.exe`` etc.) and returns ``(pid, process_name)`` pairs.
    The caller's own PID and its entire ancestor chain are excluded so the
    running ``argus update`` invocation never reports itself — this matters
    on Windows where the setuptools .exe launcher (``argus.exe``) is a
    separate process from the Python interpreter it loads (``python.exe``).
    ...
    # setuptools-generated argus.exe launcher is a separate native process
    ...
    #      argus.exe sitting *under* a non-Argus parent (e.g. a Argus
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "argus.exe"
    lines = ["✗ Another argus.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Argus Desktop, exit any open `argus` REPLs, and")
...
def _quarantine_running_hermes_exe(
    ...
    """Pre-empt Windows file lock on the running ``argus.exe``.
    ...
    when ``argus update`` runs, ``argus.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``argus.exe.old.<unix-ms>`` first. uv then writes
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``argus.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``argus.exe`` is the running process —
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``argus.exe.old.*`` left by prior updates.
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``argus*.exe`` shims
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    #
    # Quarantine the running ``argus.exe`` first: ``--reinstall -e .``
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    argus.exe concurrent-instance guard does not see them. They still import
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # On Windows, abort early if another argus.exe is holding the venv shim
...
def cmd_dashboard(args):
    ...
    # ── Project config → ARGUS_HOME sync ───────────────────────────────
    # The project-root config.yaml is the source of truth (git-tracked, travels
    # across machines). Copy it over <ARGUS_HOME>/config.yaml BEFORE any
    ...
    if (
        ...
        # ARGUS_HOME.  We must resolve the root explicitly instead of just
        # dropping ARGUS_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV ARGUS_HOME=/opt/data`), so an unset
        # ARGUS_HOME falls back to $HOME/.argus = /opt/data/.argus — an
...
def main():
    ...
    # Sweep stale ``argus.exe.old.*`` quarantine files left by previous
