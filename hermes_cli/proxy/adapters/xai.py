...
# xAI's public API is OpenAI-compatible for the endpoints Argus commonly
# uses. The Responses endpoint is included because Argus' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Argus-managed OAuth credentials."""

    auth_hint = "argus auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`argus auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`argus auth reset xai-oauth` or re-authenticate with "
                    "`argus auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `argus auth add xai-oauth --type oauth`."
            )
