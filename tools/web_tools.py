...
Backend is selected during ``argus tools`` setup (web.backend in config.yaml).
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Argus config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Argus' config/.env layer (``argus config set``, ``argus tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.argus/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``argus tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `argus tools` repaint.
        try:
