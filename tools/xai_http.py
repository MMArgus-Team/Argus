...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``argus tools`` repaint, tool-registration scans,
    ...
    2. ``~/.argus/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.argus/.env`` first, then ``os.environ``.
...
def hermes_xai_user_agent() -> str:
    """Return a stable Argus-specific User-Agent for xAI HTTP calls."""
    try:
        from hermes_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Argus-Agent/{__version__}"
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    stored in ``~/.argus/.env`` (the standard Hermes location) are honored —
