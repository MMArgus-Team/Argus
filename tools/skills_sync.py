...
Copies bundled skills from the repo's skills/ directory into ~/.argus/skills/
...
The manifest lives at ~/.argus/skills/.bundled_manifest.
"""
...
# and by the installer's `--no-skills` flag (the default ~/.argus profile).
# When present in HERMES_HOME, sync_skills() is a no-op so neither the
# installer, `argus update`, nor a direct sync re-injects bundled skills.
...
def _get_bundled_dir() -> Path:
    ...
    Checks ARGUS_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.argus/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.argus/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.argus/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.argus) that wrote the
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.argus/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `argus update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `argus skills reset {skill_name}` "
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``~/.argus/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # ``shutil.rmtree(~/.argus)`` into a loud, recoverable ``ValueError``
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `argus skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `argus update` runs "
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``argus update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``argus update`` keeps because the user
    edited them locally.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `argus skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    installer, ``argus update``, and any direct sync stop seeding bundled
    ...
    try:
        if enabled:
            ...
            changed = existed
            message = (
                "Opted back in. The next `argus update` (or `argus skills "
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.argus/skills/ ...")
