            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.argus/{rel}"
                        result.append({
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.argus``,
    SSH ``<remote_home>/.argus``, etc.) and whether translation is wanted.
