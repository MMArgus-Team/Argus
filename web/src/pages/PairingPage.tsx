...
export default function PairingPage() {
  ...
  const handleApprove = async (user: PairingUser) => {
    ...
    try {
      ...
      showToast(t.pairing.errorToast.replace("{error}", String(e)), "error");
    } finally {
      setApproving(null);
    }
  };
  ...
  const handleClearPending = async () => {
    if (!window.confirm(t.pairing.confirmClear)) return;
    setClearing(true);
    try {
    ...
    onDelete: useCallback(
      async (key: string) => {
        ...
        try {
          ...
          showToast(
            t.pairing.revokedToast.replace("{name}", user ? getUserLabel(user) : user_id),
            "success",
          );
          loadPairing();
        } catch (e) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <DeleteConfirmDialog
        ...
        title={t.pairing.revokeTitle}
        description={
          pendingRevokeUser
            ? t.pairing.revokeDescNamed.replace("{name}", getUserLabel(pendingRevokeUser))
            : t.pairing.revokeDesc
        }
      ...
      <div className="flex flex-col gap-3">
        ...
        >
          ...
          {t.pairing.pendingRequests.replace("{count}", String(pending.length))}
        </H2>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 text-xs text-muted-foreground">
                    ...
                    {typeof user.age_minutes === "number" && (
                      <span>{t.pairing.minutesAgo.replace("{minutes}", String(user.age_minutes))}</span>
                    )}
                  </div>
                </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        >
          ...
          {t.pairing.approvedUsers.replace("{count}", String(approved.length))}
        </H2>
