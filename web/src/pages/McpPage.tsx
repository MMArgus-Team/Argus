...
export default function McpPage() {
  ...
  const loadServers = useCallback(() => {
    return api
      ...
      .catch((e) => showToast(t.mcp.errorToast.replace("{error}", String(e)), "error"));
  }, [showToast, t]);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      showToast(t.mcp.addFailed.replace("{error}", String(e)), "error");
    } finally {
      setCreating(false);
    }
  };
  ...
  const handleTest = async (server: McpServer) => {
    ...
    try {
      ...
      if (result.ok) {
        showToast(t.mcp.toolsCount.replace("{name}", server.name).replace("{count}", String(result.tools.length)), "success");
      } else {
        showToast(t.mcp.testFailed.replace("{name}", server.name).replace("{error}", result.error ?? "Failed"), "error");
      }
    } catch (e) {
      showToast(t.mcp.errorToast.replace("{error}", String(e)), "error");
    } finally {
      setTesting(null);
    }
  };
    ...
    onDelete: useCallback(
      async (serverName: string) => {
        try {
          ...
          showToast(t.mcp.deleteToast.replace("{name}", truncateText(serverName, 30)), "success");
          setTestResults((prev) => {
            const next = { ...prev };
  ...
  const runInstall = useCallback(
    async (entry: McpCatalogEntry, envMap: Record<string, string>) => {
      ...
      try {
        ...
        showToast(t.mcp.installFailed.replace("{error}", String(e)), "error");
      } finally {
        setInstallingName(null);
      }
    },
    [loadServers, loadCatalog, showToast, t],
  );
  ...
  const handleInstallSubmit = () => {
    ...
    if (missing.length > 0) {
      showToast(t.mcp.requiredField.replace("{prompt}", missing[0].prompt), "error");
      return;
    }
      ...
      <DeleteConfirmDialog
        ...
        description={
          serverDelete.pendingId
            ? t.mcp.removeDescNamed.replace("{name}", truncateText(serverDelete.pendingId, 40))
            : t.mcp.removeDesc
        }
        ...
        >
          ...
          >
            ...
            <div className="p-5 grid gap-4">
              ...
              <div className="grid gap-2">
                <Label htmlFor="mcp-env">{t.mcp.envLabel}</Label>
                <textarea
                  id="mcp-env"
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 text-xs text-muted-foreground">
                    ...
                    {envCount > 0 && (
                      <span>
                        {t.mcp.envVars.replace("{count}", String(envCount))}
                      </span>
                    )}
                  </div>
                  {result && (
                      ...
                      {result.ok ? (
                        <p className="text-success">
                          {result.tools.length === 0
                            ...
                            : t.mcp.toolsList.replace("{tools}", result.tools
                                .map((tool) => tool.name)
                                .join(", "))}
                        </p>
                      ) : (
              ...
              <CardContent className="flex items-start gap-4 py-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    <Badge tone="outline">{t.mcp.authLabel.replace("{type}", entry.auth_type)}</Badge>
