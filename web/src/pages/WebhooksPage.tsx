...
export default function WebhooksPage() {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      try {
        ...
        if (st.exit_code !== 0 && st.exit_code !== null) {
          ...
          setRestartError(t.webhooks.restartFailedExit.replace("{code}", String(st.exit_code)));
          showToast(
            t.webhooks.restartFailedManual.replace("{code}", String(st.exit_code)),
            "error",
          );
        } else {
          setRestartMessage(null);
  ...
  const handleRestart = useCallback(async () => {
    ...
    try {
      ...
      showToast(t.webhooks.restartFailed.replace("{error}", String(e)), "error");
    } finally {
      setRestarting(false);
    }
  }, [loadWebhooks, showToast, watchRestartOutcome, t]);
  ...
  const handleEnableWebhooks = useCallback(async () => {
    ...
    try {
      ...
      if (result.restart_started) {
        ...
        setRestartMessage(null);
        setRestartNeeded(true);
        setRestartError(t.webhooks.restartFailedDetail.replace("{detail}", detail));
        showToast(t.webhooks.enabledRestartFailed.replace("{detail}", detail), "error");
      }
    } catch (e) {
      showToast(t.webhooks.enableFailed.replace("{error}", String(e)), "error");
    } finally {
      setEnabling(false);
    }
  }, [loadWebhooks, showToast, watchRestartOutcome, t]);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      showToast(t.webhooks.createFailed.replace("{error}", String(e)), "error");
    } finally {
      setCreating(false);
    }
  };
  ...
  const handleToggleEnabled = useCallback(
    async (subName: string, nextEnabled: boolean) => {
      ...
      try {
        ...
        showToast(
          nextEnabled
            ? t.webhooks.enabledToast.replace("{name}", subName)
            : t.webhooks.disabledToast.replace("{name}", subName),
          "success",
        );
        loadWebhooks();
      } catch (e) {
        showToast(t.webhooks.errorToast.replace("{error}", String(e)), "error");
      } finally {
        setTogglingName(null);
      }
    },
    [loadWebhooks, showToast, t],
  );
    ...
    onDelete: useCallback(
      async (name: string) => {
        try {
          ...
          showToast(t.webhooks.deletedToast.replace("{name}", name), "success");
          loadWebhooks();
        } catch (e) {
          showToast(t.webhooks.errorToast.replace("{error}", String(e)), "error");
          throw e;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <DeleteConfirmDialog
        ...
        description={
          pendingName
            ? t.webhooks.deleteDescNamed.replace("{name}", pendingName)
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <span className="text-muted-foreground">
                  {t.webhooks.receiverDisabledDesc}
                </span>
              </div>
            </div>
            <Button
        ...
        <Card className="border-warning/50">
          <CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-2 text-sm">
              ...
              <span>
                {restartError ?? t.webhooks.restartNeededDefault}
              </span>
            </div>
            <Button
              size="sm"
      ...
      <div className="flex flex-col gap-3">
        ...
        >
          ...
          {t.webhooks.subscriptionsCount.replace("{count}", String(subscriptions.length))}
        </H2>
