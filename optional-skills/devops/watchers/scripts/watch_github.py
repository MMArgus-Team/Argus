...
Usage (via cron with --no-agent):
    ...
    argus cron create argus-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ARGUS_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name argus-issues --repo MMArgus-Team/Argus --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Argus .env file
(``${ARGUS_HOME:-~/.argus}/.env``) to avoid the 60 req/hr
