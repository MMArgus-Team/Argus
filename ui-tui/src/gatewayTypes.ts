...
export type GatewayEvent =
  ...
  | {
      // Main-agent turns carry no payload. Sub-agent turns (monitor SPEAK /
      // deep-research) carry a `source` tag + routing/label fields so the TUI
      // can render them as standalone bubbles instead of hijacking the main turn.
      payload?: {
        source?: string
        request_id?: string
        monitor_id?: string
  ...
  | {
      payload: { rendered?: string; text?: string; source?: string; request_id?: string; monitor_id?: string }
      session_id?: string
      type: 'message.delta'
    }
  | {
