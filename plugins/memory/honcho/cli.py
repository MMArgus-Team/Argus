...
def cmd_sync(args) -> None:
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'argus honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'argus honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `argus update` -- no output, no exceptions.
    """
    try:
        from hermes_cli.profiles import list_profiles
        profiles = list_profiles()
...
def cmd_setup(args) -> None:
    ...
    if not is_local:
        ...
        if use_oauth:
            ...
            try:
                ...
                print("  Re-run 'argus honcho setup' to retry, or choose an API key instead.\n")
                return
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'argus honcho setup' again once you have a key.\n")
                return
    ...
    try:
        ...
        print("  Run: argus config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    argus honcho status     -- show full config")
    print("    argus honcho mode       -- change recall/observation mode")
    print("    argus honcho tokens     -- tune context and dialectic budgets")
    print("    argus honcho peer       -- update peer names")
    print("    argus honcho map <name> -- map this directory to a session name\n")
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: argus honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'argus honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'argus honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: argus honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    Hermes' identity in Honcho. Seed with 'argus honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: argus honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: argus honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("\n  Set with: argus honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'argus honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'argus honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    argus honcho identity --show        — show both peer representations")
        print("    argus honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    ...
    if has_key:
        ...
        print("  2. Run:  argus honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'argus honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'argus honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  argus honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        for f in user_files:
            print("    argus honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'argus honcho setup' first, then re-run this step.")
    else:
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'argus honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    argus honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  argus honcho identity <path/to/SOUL.md>")
    ...
    print("              Map a custom name:  argus honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. argus honcho setup              — configure API key (required)")
        print("  2. argus honcho migrate            — re-run this walkthrough")
    else:
        print("  1. argus honcho status             — verify Honcho connection")
        print("  2. hermes                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. argus honcho identity --show    — verify AI peer representation")
        print("  4. argus honcho tokens             — tune context and dialectic budgets")
        print("  5. argus honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'argus memory setup'...\n")
...
def register_cli(subparser) -> None:
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to argus memory setup)",
    )
