 ...
 *
 * The desktop main process can pass ARGUS_DASHBOARD_SESSION_TOKEN when it
...
async function fetchPublicText(url, options = {}) {
  ...
  if (protocol !== 'http:' && protocol !== 'https:') {
    throw new Error(`Unsupported Argus backend URL protocol: ${protocol}`)
  }
 ...
 */
async function adoptServedDashboardToken(baseUrl, spawnToken, { childAlive, label = 'Argus backend', ...options }) {
