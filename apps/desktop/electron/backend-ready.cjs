...
const _READY_RE = /^ARGUS_DASHBOARD_READY port=(\d+)/m
 ...
 * ARGUS_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS env override (for users on slow
...
function resolvePortAnnounceTimeoutMs(env = process.env) {
  const parsed = Number(env.ARGUS_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS)
...
/**
 * Watch a child process's stdout for the `ARGUS_DASHBOARD_READY port=<N>`
