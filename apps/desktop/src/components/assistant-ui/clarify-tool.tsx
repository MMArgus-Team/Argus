...
import { $clarifyRequest, clearClarifyRequest, registerInlineClarifyAnchor } from '@/store/clarify'
...
function ClarifyToolPending({ args }: ToolCallMessagePartProps) {
  ...
  // Claim this request id so the standalone ClarifyDialog overlay stands down —
  // this inline row owns the UI. Released on unmount / when the id changes, so a
  // raw clarify.request with no inline row falls through to the overlay instead.
  useEffect(() => {
    if (!matchingRequest?.requestId) {
      return
    }

    return registerInlineClarifyAnchor(matchingRequest.requestId)
