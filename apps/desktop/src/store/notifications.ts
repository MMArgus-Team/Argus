...
export function notifyError(error: unknown, fallback: string, options?: { durationMs?: number }): string {
  ...
  return notify({
    ...
    // Error notices are sticky by default (defaultDuration=0). Callers that want
    // an auto-dismissing error (e.g. the monitor "no video stream" toast) pass
