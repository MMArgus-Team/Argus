...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        "",
        "Hermes ships with a large built-in skill library copied into `~/.argus/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Hermes also syncs bundled skills on `argus update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.argus/skills/` tree, it is still shipped with Hermes; restore it with `argus skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with hermes-agent — install via argus skills install official/<category>/<skill>"',
        "---",
        "",
