...
def should_defer_turn_persistence(agent, *, direct_tool_turn: bool = False) -> bool:
    """Return whether canonical writes must wait for turn finalization.

    A multimodal turn can only be classified as a successful ephemeral control
    handoff after its model-selected tool has returned.  Writing the inbound
    user row or an intermediate tool protocol block before that point makes the
    append-only SessionDB impossible to clean up.  Defer those turns in the
    conversation core, rather than relying on a gateway-only private flag.

    Ordinary multimodal turns still commit their complete message sequence in
    ``finalize_turn``; only a successful pure control handoff is reduced back to
    the prior prefix there.
    """
    return bool(
        direct_tool_turn
        or getattr(agent, "_defer_current_turn_persistence", False)
        or getattr(agent, "_ephemeral_internal_turn", False)
        or getattr(agent, "_multimodal_session", False)
    )
...
def build_turn_context(
    ...
    *,
    ...
    # Restore any turn-scoped fallback before publishing the live runtime to the
    # process-global auxiliary registry. Both operations are local (no model
    # call), and direct tool turns still need the correct session credentials
    # while their pre/post-tool hooks execute.
    ...
    # set_live_watcher reports "simple"/"background" here so the loop can
    # decide whether to mute the follow-up turn (background = housekeeping-like,
    # simple = must voice the answer). Reset each turn; the tool sets it during
    # execution and the post-tool-execution promotion consults it.
    agent._mm_route_last_mode = None
    ...
    if not direct_tool_turn and agent.api_mode != "anthropic_messages":
        try:
    ...
    if not getattr(agent, "_ephemeral_internal_turn", False):
    ...
    if (not direct_tool_turn
            and not getattr(agent, "_ephemeral_internal_turn", False)
    ...
    # exists. Direct tool turns defer this decision to their finalizer because a
    # successful control action may be deliberately ephemeral; a failed action
    # is persisted there with its complete response. A gateway may likewise
    # defer a worker-owned reply until the worker has a complete A.
    if not should_defer_turn_persistence(
        agent, direct_tool_turn=direct_tool_turn
    ):
        try:
    ...
    if (agent._memory_manager and not direct_tool_turn
            and not getattr(agent, "_ephemeral_internal_turn", False)):
        try:
