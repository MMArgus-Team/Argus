...
1. **Built-in providers** (``BUILTIN_STT_PROVIDERS`` in
   ...
   ``ARGUS_LOCAL_STT_COMMAND`` is preserved via the built-in
   ``local_command`` path.
...
``~/.argus/plugins/transcription/<name>/`` (user-installed).
...
class TranscriptionProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``argus tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``argus setup`` for
