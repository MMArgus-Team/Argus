...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``argus model`` / ``hermes auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Argus process
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            if self.provider == "anthropic":
                ...
                # refresh_token — single-use tokens consumed by another Argus
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Argus process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Argus profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `argus model` / `hermes auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `argus model` ->
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `argus auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Argus PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `argus setup` ("Claude
        ...
        # that masquerade.  Prefer ~/.argus/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `argus setup` writes.
        _env_file = load_env()
        ...
        # Argus auth store or env vars, so resolve them here.
        ...
        # MiniMax OAuth tokens live in ~/.argus/auth.json providers.minimax-oauth.
        ...
        # the Argus auth store.  Without this gate the removal is instantly
        ...
        # Argus owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``argus model`` -> xAI Grok OAuth,
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.argus/.env over os.environ — the user's config file is the
    # authoritative source for Argus credentials. Stale env vars from parent
    ...
    # won't be re-seeded from the user's shell environment or ~/.argus/.env.
    ...
    if provider == "openrouter":
        # Prefer ~/.argus/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # File-backed singletons (device-code OAuth, claude_code) and Argus
