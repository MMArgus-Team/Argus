...
def _prefers_messages_transport(model: str, base_url: str) -> bool:
    ...
    Transport signal precedence (endpoint > model name):
      1. endpoint ends with ``/chat/completions`` (or ``/v1/chat/completions``)
         ...
         OpenAI-compatible request format (e.g. doc.devops Luna 在 chat/
...
def build_submodule_client(
    *, provider: str, base_url: str, api_key: str, model: str,
    ...
    # Monitor path is explicit opt-in via config; any other submodule pointed
    # at a xhs MaaS endpoint auto-adds the api-key header (URL-based detection).
    _url_lower = base_url.lower()
    _is_maas = "maas.devops.xiaohongshu.com" in _url_lower or "/slytherinai-" in _url_lower
    if (label == "monitor" and send_api_key_header) or _is_maas:
        client_kwargs["default_headers"] = {"api-key": resolved_api_key}
        if _is_maas and label != "monitor":
            log.info("[multimodal] %s client: MaaS gateway auto-added api-key header", label)
...
class HermesClientFactory(LLMClientFactory):
    ...
    def recall_client(self) -> Tuple[Any, str]:
        ...
        Typical split:
          ...
          * recall decide/distill @ Luna @ doc.devops/v1/chat/completions
    ...
    def _memory_client_from_config(
        ...
        if cfg and (base_url or "").strip():
            ...
            # ★ MaaS 网关 (小红书 slytherinai-* / maas.devops.xiaohongshu.com) 要求
            #   Bearer + api-key 双 header 认证. build_submodule_client 里的
            #   send_api_key_header 只对 label='monitor' 生效, memory 段完全被忽略 → 401.
            #   这里按 URL 自动补 api-key header (硬编码路径, config 无需 opt-in).
            _url_lower = base_url.lower()
            if "maas.devops.xiaohongshu.com" in _url_lower or "/slytherinai-" in _url_lower:
                _client_kwargs["default_headers"] = {"api-key": _mem_key.strip()}
                log.info("[multimodal] %s backend: MaaS gateway detected, adding api-key header", role_label)
