...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `argus doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "ARGUS_HOME", hermes_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("ARGUS_KANBAN_TASK", "probe")
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("ARGUS_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        monkeypatch.setattr(doctor_mod, "ARGUS_HOME", home)
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale ARGUS_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"ARGUS_MAX_ITERATIONS={ghost}\n")
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("ARGUS_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("ARGUS_MAX_ITERATIONS", raising=False)
    ...
    def test_detects_drift_warn_only(self, monkeypatch, tmp_path):
        ...
        assert "ARGUS_MAX_ITERATIONS=90" in out
        assert "shadows" in out
        # Warn-only must NOT mutate .env.
        assert "ARGUS_MAX_ITERATIONS=90" in (hermes_home / ".env").read_text(encoding="utf-8")
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale ARGUS_MAX_ITERATIONS" in out
        env_after = (hermes_home / ".env").read_text(encoding="utf-8")
        assert "ARGUS_MAX_ITERATIONS" not in env_after
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`argus doctor` must not hand users `npm audit fix --workspace <name>`:
