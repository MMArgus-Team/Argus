...
- Bare `argus setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `argus setup --quick` runs the narrower "fill in missing items" flow.
- `argus setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
class TestExistingInstallDefault:
    """Bare `argus setup` on an existing install = full reconfigure wizard."""
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`argus setup --reconfigure` behaves the same as bare `argus setup`."""
