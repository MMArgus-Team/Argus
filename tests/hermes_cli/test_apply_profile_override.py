...
def _run_apply_profile_override(
    ...
    Returns the value of os.environ["ARGUS_HOME"] after the call,
    or None if unset.
    """
    ...
    if hermes_home is not None:
        monkeypatch.setenv("ARGUS_HOME", hermes_home)
    else:
    ...
    return os.environ.get("ARGUS_HOME")
...
class TestApplyProfileOverrideHermesHomeGuard:
    ...
    def test_sudo_explicit_profile_resolves_invoking_users_profile(self, tmp_path, monkeypatch):
        ...
        profile_dir = user_home / ".argus" / "profiles" / "elias"
        profile_dir.mkdir(parents=True, exist_ok=True)
        (root_home / ".argus").mkdir(parents=True, exist_ok=True)
    ...
    def test_hermes_home_unset_default_profile_no_redirect(self, tmp_path, monkeypatch):
        ...
        assert os.environ.get("ARGUS_HOME") is None
...
class TestSupervisedChildIgnoresStickyProfile:
    ...
    def test_supervised_child_does_not_follow_active_profile(
        ...
        """ARGUS_S6_SUPERVISED_CHILD + active_profile=briefer must NOT redirect.
    ...
    def test_non_supervised_run_still_follows_active_profile(
        ...
        """Without the sentinel, a normal `argus gateway run` still honors
    ...
    def test_supervised_named_profile_flag_still_wins(self, tmp_path, monkeypatch):
        ...
        result = os.environ.get("ARGUS_HOME")
        assert result is not None
        assert result.endswith("coder")
