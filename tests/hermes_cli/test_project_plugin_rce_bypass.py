...
Execution via the ``ARGUS_ENABLE_PROJECT_PLUGINS`` bypass in the web
...
1. ``hermes_cli.web_server._discover_dashboard_plugins`` opted into
   the untrusted ``./.argus/plugins/`` source via
   ``os.environ.get("ARGUS_ENABLE_PROJECT_PLUGINS")`` — truthy for
...
# ---------------------------------------------------------------------------
# Layer 1 — ARGUS_ENABLE_PROJECT_PLUGINS env gate uses truthy semantics.
# ---------------------------------------------------------------------------
...
class TestProjectPluginsEnvGate:
    ...
    def test_falsy_values_keep_project_plugins_disabled(
        ...
        if value == "":
            monkeypatch.delenv("ARGUS_ENABLE_PROJECT_PLUGINS", raising=False)
        else:
            monkeypatch.setenv("ARGUS_ENABLE_PROJECT_PLUGINS", value)

        plugins = web_server._get_dashboard_plugins(force_rescan=True)
        names = {p["name"] for p in plugins}
        assert "evil" not in names, (
            f"ARGUS_ENABLE_PROJECT_PLUGINS={value!r} must NOT enable the "
...
class TestEndToEndPocBlocked:
    ...
    ``ARGUS_ENABLE_PROJECT_PLUGINS=0`` (so the operator believed the
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ARGUS_ENABLE_PROJECT_PLUGINS", "0")
