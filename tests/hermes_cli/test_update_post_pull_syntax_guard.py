"""Tests for the post-pull syntax guard in ``argus update``.
...
markers in ``hermes_cli/config.py``; users who ran ``argus update`` in
...
# release; if a future ``argus update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
...
def test_production_tree_passes_syntax_guard():
    ...
    assert ok is True, (
        ...
        f"argus update would brick users. Error: {error}"
    )
