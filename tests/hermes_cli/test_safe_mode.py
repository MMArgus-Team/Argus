...
_VARS = ("ARGUS_SAFE_MODE", "ARGUS_IGNORE_USER_CONFIG", "ARGUS_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        ...
        assert os.environ.get("ARGUS_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under ARGUS_SAFE_MODE=1."""

    def test_discovery_skipped(self, monkeypatch):
        monkeypatch.setenv("ARGUS_SAFE_MODE", "1")
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("ARGUS_SAFE_MODE", raising=False)
        from hermes_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under ARGUS_SAFE_MODE=1."""
