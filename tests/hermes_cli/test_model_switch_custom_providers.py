...
def test_list_authenticated_providers_includes_custom_providers(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "ARGUS_OVERLAYS", {})
...
def test_custom_providers_uses_live_models_for_multi_model_endpoint(monkeypatch):
    ...
    """
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr("hermes_cli.providers.ARGUS_OVERLAYS", {})
...
def test_live_discovery_does_not_delete_explicitly_configured_model(monkeypatch):
    """A hand-configured model must survive an endpoint that omits it.

    Live ``/models`` is the source of truth for what a gateway *offers*, but it
    is not an allow-list: endpoints routinely serve models their catalog never
    lists. ``open.bigmodel.cn`` is the reported case — its ``/models`` has no
    ``glm-5v-turbo``, yet the endpoint serves it and the user configured it.

    Replacing the configured list with the live one made that model visible only
    while it happened to be the *current* model (the ``current_model`` post-pass
    re-injects that single entry). Selecting any other model dropped it from the
    picker, so it could never be selected again — a one-way trap.
    """
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr("hermes_cli.providers.ARGUS_OVERLAYS", {})
    monkeypatch.setattr(
        "hermes_cli.models.fetch_api_models",
        lambda api_key, base_url: ["glm-4.5", "glm-4.5-air", "glm-5"],
    )
    ...
    def models_for(current_model: str) -> list:
        providers = list_authenticated_providers(
            current_provider="custom:open.bigmodel.cn",
        ...
        row = next(
            (
                p
                for p in providers
                if "bigmodel" in str(p.get("api_url", "")).lower()
            ),
            None,
        )
        assert row is not None, "custom provider row missing"
        return row["models"]

    # The regression: visible while selected, gone after switching away.
    assert "glm-5v-turbo" in models_for("glm-5v-turbo")
    assert "glm-5v-turbo" in models_for("glm-4.5-air"), (
        "explicitly configured model must not vanish once another model is selected"
    )
    assert "glm-5v-turbo" in models_for("glm-5")

    # Live catalog is still merged in, and the explicit entry leads.
    merged = models_for("glm-5")
    assert merged[0] == "glm-5v-turbo"
    for live in ("glm-4.5", "glm-4.5-air", "glm-5"):
        assert live in merged
    assert len(merged) == len(set(merged)), "merge must not duplicate"


def test_live_discovery_merge_keeps_user_providers_model(monkeypatch):
    """Same guarantee for section 3 (``providers:`` dict), not just section 4."""
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr("hermes_cli.providers.ARGUS_OVERLAYS", {})
    monkeypatch.setattr(
        "hermes_cli.models.fetch_api_models",
        lambda api_key, base_url: ["listed-a", "listed-b"],
    )
    ...
    row = next(
        (p for p in providers if p.get("api_url") == "https://my-endpoint.example.com/v1"),
        None,
    )

    assert row is not None
    assert row["models"][0] == "unlisted-vision-model"
    assert "listed-a" in row["models"]
