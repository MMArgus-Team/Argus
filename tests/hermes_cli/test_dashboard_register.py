"""Tests for ``argus dashboard register``.
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://portal.nousresearch.com",
        ...
        # (idempotency key) and once for ARGUS_DASHBOARD_PORTAL_URL. Route by
        ...
        def fake_get_env(key):
            if key == "ARGUS_DASHBOARD_OAUTH_CLIENT_ID":
    ...
    def test_writes_client_id_and_posts_generated_name(self, capsys):
        ...
        assert saved["ARGUS_DASHBOARD_OAUTH_CLIENT_ID"] == "agent:selfhost-1"
        assert "ARGUS_DASHBOARD_PORTAL_URL" not in saved
    ...
    def test_non_default_portal_is_persisted(self, capsys):
        ...
        assert (
            saved["ARGUS_DASHBOARD_PORTAL_URL"]
...
class TestIdempotentRerun(TestHappyPath):
    ...
    ``existing_client_id`` (the ARGUS_DASHBOARD_OAUTH_CLIENT_ID a prior run
    ...
    def test_stale_id_falls_through_to_create_prints_registered(self, capsys):
        ...
        assert saved["ARGUS_DASHBOARD_OAUTH_CLIENT_ID"] == "agent:selfhost-new"
...
class TestCustomPortalPersistence:
    """`--portal-url` / ARGUS_DASHBOARD_PORTAL_URL is persisted to .env.
    ...
    def _run(self, *, args, portal, existing_portal):
        ...
        ARGUS_DASHBOARD_PORTAL_URL (None = not present in .env).
        """
        response = {
            "client_id": "agent:selfhost-1",
            "id": "selfhost-1",
        ...
        def fake_get_env_value(key, *a, **kw):
            if key == "ARGUS_DASHBOARD_PORTAL_URL":
        ...
        with patch(
            ...
            # The ambient process env may carry ARGUS_DASHBOARD_PORTAL_URL
            # (e.g. staging dev shells); drop it so `custom_portal_supplied`
            # is driven solely by the args.portal_url under test.
            dr.os.environ.pop("ARGUS_DASHBOARD_PORTAL_URL", None)
            dr.cmd_dashboard_register(args)
        return saved
    ...
    def test_explicit_custom_url_persisted_when_var_absent(self, capsys):
        ...
        assert saved["ARGUS_DASHBOARD_PORTAL_URL"] == "https://preview.example.com"
    ...
    def test_explicit_custom_url_updates_existing_in_place(self, capsys):
        ...
        assert (
            saved["ARGUS_DASHBOARD_PORTAL_URL"] == "https://new-preview.example.com"
        )
    ...
    def test_explicit_custom_url_persisted_even_when_equals_default(self, capsys):
        ...
        assert (
            saved["ARGUS_DASHBOARD_PORTAL_URL"] == "https://portal.nousresearch.com"
        )
...
class TestPublicUrlPersistence:
    """`--redirect-uri` derives & persists ARGUS_DASHBOARD_PUBLIC_URL in .env.
    ...
    ARGUS_DASHBOARD_PUBLIC_URL, so the value that's actually consumed is the
    ORIGIN (scheme://host). We derive the origin from the supplied redirect URI
    and persist THAT as ARGUS_DASHBOARD_PUBLIC_URL — the var the runtime reads
    ...
    def _run(self, *, args, existing_public=None):
        ...
        ARGUS_DASHBOARD_PUBLIC_URL (None = not present in .env).
        """
        response = {
            "client_id": "agent:selfhost-1",
            "id": "selfhost-1",
        ...
        def fake_get_env_value(key, *a, **kw):
            if key == "ARGUS_DASHBOARD_PUBLIC_URL":
    ...
    def test_origin_derived_from_full_callback_path(self, capsys):
        ...
        assert saved["ARGUS_DASHBOARD_PUBLIC_URL"] == "https://hermes.example.com"
        # The full callback path must NOT be persisted verbatim (would double
        # the path at serve time).
        assert "/auth/callback" not in saved["ARGUS_DASHBOARD_PUBLIC_URL"]
    ...
    def test_origin_preserves_port(self, capsys):
        ...
        assert saved["ARGUS_DASHBOARD_PUBLIC_URL"] == "https://hermes.example.com:8443"
    ...
    def test_public_url_updates_existing_in_place(self, capsys):
        ...
        assert saved["ARGUS_DASHBOARD_PUBLIC_URL"] == "https://new.example.com"
    ...
    def test_public_url_equal_to_existing_is_noop(self, capsys):
        ...
        assert "ARGUS_DASHBOARD_PUBLIC_URL" not in saved
