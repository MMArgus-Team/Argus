...
class TestResolveVerifyFallback:
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("ARGUS_CA_BUNDLE", raising=False)
    ...
    def test_missing_hermes_ca_bundle_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.setenv("ARGUS_CA_BUNDLE", "/nonexistent/hermes-ca.pem")
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ARGUS_HOME", str(hermes_home))
    monkeypatch.setenv("ARGUS_AGENT_USE_LEGACY_SESSION_KEYS", "true")
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `argus auth add nous`
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `argus auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    assert "argus auth add nous" in message.lower()
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect ARGUS_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("ARGUS_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without ARGUS_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.argus/shared/`` across CI runs.
    """
    from hermes_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("ARGUS_SHARED_AUTH_DIR", raising=False)
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `argus auth add nous
...
def test_shared_store_survives_across_profile_switch(
    ...
    monkeypatch.setenv("ARGUS_HOME", str(profile_a))
    ...
    monkeypatch.setenv("ARGUS_HOME", str(profile_b))
