...
class TestFinalizeSessionPersist:
    ...
    def test_multimodal_dependents_stop_before_memory_backend(self):
        """Voice/Watcher must stop before the backend resources they use."""
        from tui_gateway.server import _finalize_session
        ...
        session = _make_session(agent=None, history=[])
        session.update({
            "_mm_voice_agent": _Resident("voice"),
            "_mm_monitor_engine": _Resident("monitor"),
            "_mm_live_watcher_agent": _Resident("watcher"),
            "_mm_memory_backend": _Resident("memory"),
        })

        _finalize_session(session)

        assert calls == ["voice", "monitor", "watcher", "memory"]

    def test_monitor_start_racing_finalize_cannot_escape_session(self):
        """A Monitor published before a blocking start is stopped by finalize."""
        ...
        class _BlockingMonitorEngine:
            ...
            def is_healthy(self):
                return self.healthy and not self.stopped
        ...
        with patch(
            "agent.multimodal.monitor_engine.MonitorEngine",
            _BlockingMonitorEngine,
        ):
            starter = threading.Thread(
                target=lambda: outcome.append(
                    _maybe_start_monitor_engine("sid-race", session, frame_buffer)
                )
            )
            starter.start()
            assert entered_start.wait(timeout=2.0)
            assert session.get("_mm_monitor_engine") is instances[0]
