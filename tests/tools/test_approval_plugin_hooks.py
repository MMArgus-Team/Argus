...
def isolated_session(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("ARGUS_SESSION_KEY", session_key)
...
class TestCliPathFiresHooks:
    """CLI-interactive approval path: ARGUS_INTERACTIVE is set, the
...
class TestGatewayPathFiresHooks:
    """Async gateway approval path: ARGUS_GATEWAY_SESSION is set and a
