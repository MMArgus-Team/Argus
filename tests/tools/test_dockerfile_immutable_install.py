...
def test_dockerfile_keeps_mutable_state_under_opt_data() -> None:
    ...
    assert "ENV ARGUS_HOME=/opt/data" in text
    assert "ENV ARGUS_WRITE_SAFE_ROOT=/opt/data" in text
...
def test_dockerfile_disables_runtime_install_mutations() -> None:
    ...
    assert "ENV ARGUS_DISABLE_LAZY_INSTALLS=1" in text
    assert "ARGUS_TUI_DIR=/opt/hermes/ui-tui" in text
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    shared $ARGUS_HOME data volume (which a host install may also use).
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    assert f"ENV ARGUS_LAZY_INSTALL_TARGET={target}" in text
    assert target.startswith("/opt/data/"), "target must be on the durable volume"
    assert "ENV ARGUS_LAZY_INSTALL_TARGET=/opt/hermes" not in text
    ...
    assert '"$ARGUS_HOME/lazy-packages"' in stage2, (
