...
class TestCronDenyMode:
    """When ARGUS_CRON_SESSION is set and cron_mode=deny, dangerous commands are blocked."""
...
class TestCronApproveMode:
    """When ARGUS_CRON_SESSION is set and cron_mode=approve, dangerous commands pass through."""
...
class TestCronModeInteractions:
    ...
    def test_yolo_overrides_cron_deny(self, monkeypatch):
        ...
        monkeypatch.setenv("ARGUS_CRON_SESSION", "1")
        monkeypatch.setenv("ARGUS_YOLO_MODE", "1")
        monkeypatch.delenv("ARGUS_INTERACTIVE", raising=False)
        monkeypatch.delenv("ARGUS_GATEWAY_SESSION", raising=False)

        # _YOLO_MODE_FROZEN is frozen at module import time (security: prevents
        # prompt injection from runtime-setting ARGUS_YOLO_MODE). When the
...
class TestCronWithGatewayOrigin:
    ...
    cron/scheduler.py binds ARGUS_SESSION_PLATFORM via contextvars for
