...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"ARGUS_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_hermes_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.argus/.env")
        assert dangerous is True
        assert key is not None
...
class TestHermesConfigWriteProtection:
    ...
    ~/.argus/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.argus/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.argus/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.argus/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.argus/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.argus/config.yaml")
        assert dangerous is True
        assert "argus config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.argus/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.argus/config.yaml"
        )
        assert dangerous is True
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.argus/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.argus/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.argus/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.argus/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.argus/config.yaml"
        )
        assert dangerous is False

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.argus/config.yaml")
        assert dangerous is False
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.argus/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    ...
    ~/.argus/ in dangerous-command detection.
    ...
    def test_windows_hermes_home_config_folds(self, monkeypatch):
        ...
        monkeypatch.setenv("HOME", r"C:\Users\tester")
        monkeypatch.setenv("ARGUS_HOME", r"C:\Users\tester\.hermes")
        dangerous, key, _ = detect_dangerous_command(
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.argus/hermes-agent && source venv/bin/activate && python -m hermes_cli.main gateway run --replace &disown; echo done"
...
class TestLaunchctlGatewayLifecycle:
    ...
    label achieves the same effect as `argus gateway stop|restart` and
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("ARGUS_GATEWAY_SESSION", "ARGUS_CRON_SESSION",
                      "ARGUS_YOLO_MODE",
                      "ARGUS_SESSION_KEY", "ARGUS_INTERACTIVE")
        }
        os.environ.pop("ARGUS_YOLO_MODE", None)
        os.environ.pop("ARGUS_INTERACTIVE", None)
        # ARGUS_CRON_SESSION takes priority over ARGUS_GATEWAY_SESSION in
        ...
        os.environ.pop("ARGUS_CRON_SESSION", None)
        os.environ["ARGUS_GATEWAY_SESSION"] = "1"
        os.environ["ARGUS_SESSION_KEY"] = self.SESSION_KEY
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("hermes_cli.config.load_config", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"ARGUS_INTERACTIVE": "1"}, clear=False):
