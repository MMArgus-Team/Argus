...
``~/.argus/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.argus/.env`` would be told
...
class TestEndToEndRegressionGuard:
    ...
    ``~/.argus/.env`` carrying the key while ``os.environ`` does not.
