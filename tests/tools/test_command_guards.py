...
def _clean_state():
    ...
    saved = {}
    for k in ("ARGUS_INTERACTIVE", "ARGUS_GATEWAY_SESSION", "ARGUS_EXEC_ASK", "ARGUS_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ARGUS_INTERACTIVE"] = "1"
        session_key = os.getenv("ARGUS_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ARGUS_INTERACTIVE or ARGUS_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
