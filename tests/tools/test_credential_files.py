...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ARGUS_HOME": str(hermes_home)}):
        ...
        assert mounts[0]["container_path"] == "/root/.argus/token.json"
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.argus/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ARGUS_HOME": str(hermes_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.argus")

        assert mounts[0]["container_path"] == "/home/user/.argus/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.argus/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.argus/skills/cat/myskill/scripts/run.sh" in paths
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        assert "/root/.argus/cache/documents" in paths
        assert "/root/.argus/cache/audio" in paths
        assert "/root/.argus/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.argus/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        assert "/root/.argus/cache/documents" in container_paths
        assert "/root/.argus/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        ...
        assert (
            ...
            == "/root/.argus/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.argus")
            == "/home/agent/.argus/cache/images/remote.png"
        )
...
class TestIterCacheFiles:
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        assert entries[0]["container_path"] == "/root/.argus/cache/screenshots/session_abc/screen1.png"
