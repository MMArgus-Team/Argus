...
class TestSendMessageTool:
    ...
    def test_cron_duplicate_target_is_skipped_and_explained(self):
        ...
        with patch.dict(
            os.environ,
            {
                "ARGUS_CRON_AUTO_DELIVER_PLATFORM": "telegram",
                "ARGUS_CRON_AUTO_DELIVER_CHAT_ID": "-1001",
            },
            clear=False,
        ), \
    ...
    def test_mirror_receives_current_session_user_id(self):
        ...
        with patch("gateway.config.load_gateway_config", return_value=config), \
            ...
            get_session_env_mock.side_effect = lambda name, default="": {
                ...
                "ARGUS_SESSION_USER_ID": "user-123",
            }.get(name, default)
            result = json.loads(
                send_message_tool(
                    {
...
class TestSendViaAdapterStandaloneFallback:
    ...
    When the gateway runner is not in this process (e.g. ``argus cron``
    runs separately from ``argus gateway``), ``_send_via_adapter`` should
...
class TestCheckSendMessage:
    ...
    1. ``ARGUS_KANBAN_TASK`` is set (worker spawned by the kanban dispatcher
       — parent gateway is by definition running, but the worker's
       ``HERMES_HOME`` may be a profile dir without a ``gateway.pid``).
    2. ``ARGUS_SESSION_PLATFORM`` resolves to a non-empty, non-``local`` value
    ...
    def test_kanban_task_env_short_circuits_before_gateway_check(self, monkeypatch):
        """Honoring ARGUS_KANBAN_TASK must not depend on importing or calling
    ...
    def test_local_platform_falls_through_to_gateway_check(self, monkeypatch):
        """``ARGUS_SESSION_PLATFORM=local`` means CLI-style — must defer to
