...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad ARGUS_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ARGUS_GATEWAY_SESSION set, a bound session
    key, and isolated gateway queues/callbacks. Yields the session_key."""
    monkeypatch.setenv("ARGUS_GATEWAY_SESSION", "1")
    monkeypatch.delenv("ARGUS_INTERACTIVE", raising=False)
    monkeypatch.delenv("ARGUS_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("ARGUS_CRON_SESSION", "1")
    monkeypatch.delenv("ARGUS_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_hermes_allowlist_and_secret_blocks():
    ...
    env = {
        ...
        "ARGUS_HOME": "/h", "ARGUS_PROFILE": "p",
        "ARGUS_CONFIG": "/c.yaml", "ARGUS_ENV": "/e",
        # otherARGUS_* → dropped (broad prefix removed)
        "ARGUS_BASE_URL": "https://x", "ARGUS_INTERACTIVE": "1",
        "ARGUS_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("ARGUS_HOME", "ARGUS_PROFILE", "ARGUS_CONFIG", "ARGUS_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ARGUS_BASE_URL", "ARGUS_INTERACTIVE", "ARGUS_KANBAN_DB",
...
def test_env_scrub_logs_dropped_hermes_vars(caplog):
    """Dropping a non-allowlisted, non-secretARGUS_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset ARGUS_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "ARGUS_HOME": "/h",          # allowlisted → kept, not logged
        "ARGUS_BASE_URL": "https://x",   # dropped → logged
        "ARGUS_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "ARGUS_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "ARGUS_HOME" in out and "PATH" in out
    assert "ARGUS_BASE_URL" not in out and "ARGUS_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "ARGUS_BASE_URL" in msgs and "ARGUS_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "ARGUS_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no droppedARGUS_* vars."""
    import logging
