...
Build the real image and verify at runtime:
  ...
  2. PYTHONDONTWRITEBYTECODE and ARGUS_DISABLE_LAZY_INSTALLS are set
...
def test_hermes_disable_lazy_installs_and_dont_write_bytecode(
    ...
    ARGUS_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
    ...
    r = docker_exec_sh(
        ...
        'test "$ARGUS_DISABLE_LAZY_INSTALLS" = "1" && '
        'echo ENV_OK || echo ENV_MISSING',
        timeout=10,
    )
    assert "ENV_OK" in r.stdout, (
        f"expected PYTHONDONTWRITEBYTECODE=1 and "
        f"ARGUS_DISABLE_LAZY_INSTALLS=1, got: {r.stdout} stderr={r.stderr}"
    )
