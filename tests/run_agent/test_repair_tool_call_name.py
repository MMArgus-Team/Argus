...
class TestRetiredMultimodalToolName:
    """A retired model-facing name is not a spelling variant of its successor.

    ``query_multimodal`` has a broader behavioral contract than the old Recall-
    only entry.  Runtime name repair must therefore never turn a stale model
    call into the new tool behind the model's back; the normal unknown-tool
    feedback lets the model self-correct from the current schema instead.
    """

    @pytest.fixture
    def query_only_repair(self):
        from run_agent import AIAgent

        stub = SimpleNamespace(valid_tool_names={"query_multimodal"})
        return AIAgent._repair_tool_call.__get__(stub, AIAgent)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "RecallMultimodalMemory",
            "recall-multimodal-memory",
        ],
    )
    def test_legacy_name_is_not_aliased_to_query_multimodal(
        self, query_only_repair, stale_name
    ):
        assert query_only_repair(stale_name) is None

    def test_actual_new_name_remains_a_direct_match(self, query_only_repair):
        assert query_only_repair("query_multimodal") == "query_multimodal"
