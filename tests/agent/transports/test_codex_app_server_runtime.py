...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("ARGUS_HOME", "/users/alice/.argus/profiles/backend-worker")
        monkeypatch.setenv("ARGUS_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "ARGUS_KANBAN_DB",
            "/users/alice/.argus/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.argus/kanban/boards/smoke"]'
            in cmd
        )
