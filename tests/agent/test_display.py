...
class TestBuildToolPreview:
    ...
    def test_unknown_tool_no_matching_key(self):
        """Unknown tool with unrecognized keys previews from the args generically.

        BEHAVIOR CHANGE (was: returns None). Returning None meant the UIs
        rendered a bare tool name with no hint of what the call was doing —
        measured across the registry, 37 tools hit that path, and every newly
        added tool defaulted into it. `build_tool_preview` now falls back to
        `build_generic_args_preview`, so having args is enough to say
        something. None is still returned for empty/non-dict args (below).
        """
...
class TestComputerUsePreview:
    """`computer_use` is an action-discriminated tool whose informative args
    (element, coordinate, keys, app) match none of the legacy fallback keys, so
    it used to preview as a bare tool name. Phrasing now mirrors the tool's own
    approval-prompt summariser."""

    def test_capture_includes_mode_and_app(self):
        result = build_tool_preview(
            "computer_use",
            {"action": "capture", "mode": "som", "app": "Google Chrome"},
        )
        assert result == "capture som · Google Chrome"

    def test_click_by_element_index(self):
        result = build_tool_preview(
            "computer_use", {"action": "click", "element": 7, "app": "Google Chrome"}
        )
        assert result == "click #7 · Google Chrome"

    def test_click_by_coordinate(self):
        result = build_tool_preview(
            "computer_use", {"action": "click", "coordinate": [820, 410]}
        )
        assert result == "click at (820,410)"

    def test_scroll_shows_direction_and_amount(self):
        result = build_tool_preview(
            "computer_use", {"action": "scroll", "direction": "down", "amount": 5}
        )
        assert result == "scroll down x5"

    def test_key_shows_combo(self):
        result = build_tool_preview("computer_use", {"action": "key", "keys": "cmd+s"})
        assert result == "key cmd+s"

    def test_launch_app_shows_target(self):
        result = build_tool_preview(
            "computer_use", {"action": "launch_app", "name": "Tencent Meeting"}
        )
        assert result == "launch_app Tencent Meeting"

    def test_bare_action_still_previews(self):
        assert build_tool_preview("computer_use", {"action": "list_apps"}) == "list_apps"

    def test_typed_text_is_never_shown(self):
        # A typed string can be a password entered into a form. Length only.
        result = build_tool_preview(
            "computer_use", {"action": "type", "text": "hunter2"}
        )
        assert result == "type (7 chars)"
        assert "hunter2" not in result

    def test_action_missing_returns_none(self):
        assert build_tool_preview("computer_use", {"mode": "som"}) is None


class TestArgFieldClassification:
    def test_enum_and_scalar_args_are_literals(self):
        assert classify_arg_field("action", "capture") == "literal"
        assert classify_arg_field("element", 7) == "literal"
        assert classify_arg_field("annotate", True) == "literal"

    def test_payload_names_are_freeform(self):
        for name in ("content", "body", "message", "text", "old_string", "code"):
            assert classify_arg_field(name, "anything") == "freeform"

    def test_intent_prose_is_displayable(self):
        # The model describing its own action is the most useful thing to show,
        # and the bespoke branches for clarify/vision_analyze already show it.
        for name in ("question", "goal", "reason", "title"):
            assert classify_arg_field(name, "what is on screen") == "literal"

    def test_collections_are_shapes(self):
        assert classify_arg_field("urls", ["a", "b"]) == "shape"
        assert classify_arg_field("data", {"k": "v"}) == "shape"

    def test_unknown_long_string_defaults_to_hidden(self):
        # An unclassified field (e.g. from an MCP tool) that holds prose must
        # not be displayed on the strength of a guess.
        assert classify_arg_field("blob", "x" * 500) == "freeform"
        assert classify_arg_field("blob", "short-id") == "literal"


class TestGenericArgsPreview:
    def test_renders_every_informative_field(self):
        result = build_generic_args_preview(
            {"domain": "light", "service": "turn_on", "entity_id": "light.kitchen"}
        )
        assert result == "domain=light service=turn_on entity_id=light.kitchen"

    def test_leads_with_the_operation_kind(self):
        result = build_generic_args_preview({"watcher_id": "w1", "op": "create"})
        assert result.startswith("create")

    def test_payload_field_shows_length_only(self):
        result = build_generic_args_preview(
            {"group_code": "G9", "message": "my password is hunter2"}
        )
        assert "group_code=G9" in result
        assert "message(22 chars)" in result
        assert "hunter2" not in result

    def test_collection_shows_count(self):
        assert build_generic_args_preview({"urls": ["a", "b", "c"]}) == "urls[3]"

    def test_empty_values_are_skipped(self):
        assert build_generic_args_preview({"a": "", "b": None, "c": []}) is None

    def test_non_dict_returns_none(self):
        assert build_generic_args_preview({}) is None
        assert build_generic_args_preview(None) is None


class TestArgPreviewLeakPrevention:
    """The generic renderer sends arbitrary tool args to a UI, so these are the
    guards that make that safe. `redact_sensitive_text` catches credential
    SHAPES; the two things it provably does NOT catch are covered here."""

    def test_url_userinfo_password_is_masked(self):
        # Global URL redaction is deliberately off (it breaks magic-link/OAuth
        # flows), so the display layer masks userinfo per-value instead.
        result = build_generic_args_preview(
            {"url": "https://user:p4ssw0rd@internal.example.com/api"}
        )
        assert "p4ssw0rd" not in result
        assert "***" in result

    def test_prose_secret_is_hidden_by_classification(self):
        # No regex will ever recognise "my password is hunter2" as a secret.
        # It stays hidden because payload fields are never displayed.
        result = build_generic_args_preview({"message": "my password is hunter2"})
        assert "hunter2" not in result

    def test_credential_shape_in_a_literal_is_redacted(self):
        result = build_generic_args_preview(
            {"header": "Authorization: Bearer sk-ant-api03-AbCdEfGh123456"}
        )
        assert "sk-ant-api03-AbCdEfGh123456" not in result

    def test_file_content_never_reaches_the_preview(self):
        result = build_generic_args_preview(
            {"path": "/tmp/a.py", "content": "SECRET_TOKEN=hunter2"}
        )
        assert "hunter2" not in result
        assert "/tmp/a.py" in result

    def test_describe_arg_fields_omits_payload_values_entirely(self):
        # The structured payload the UI expands must carry no payload text at
        # all — not truncated, absent.
        fields = describe_arg_fields(
            {"group_code": "G9", "message": "my password is hunter2"}
        )
        by_key = {f["key"]: f for f in fields}
        assert by_key["message"]["kind"] == "freeform"
        assert "value" not in by_key["message"]
        assert by_key["message"]["chars"] == len("my password is hunter2")
        assert "hunter2" not in json.dumps(fields, ensure_ascii=False)


class TestPreviewFallbackCoverage:
    """The point of the change: having args is enough to say something."""

    def test_previously_silent_tools_now_preview(self):
        # Each of these returned None before (measured across the registry):
        # their informative args matched no entry in the old guess list.
        cases = {
            "computer_use": {"action": "capture", "mode": "som"},
            "kanban_comment": {"task_id": "T-1042", "body": "see logs"},
            "ha_get_state": {"entity_id": "light.kitchen"},
            "feishu_doc_read": {"doc_token": "doxcnAbCdEf"},
            "browser_press": {"key": "Enter"},
            "read_terminal": {"start_line": 10, "count": 50},
            "project_switch": {"project": "argus"},
        }
        for name, args in cases.items():
            assert build_tool_preview(name, args), f"{name} still has no preview"

    def test_multi_field_call_keeps_the_routing_context(self):
        # Regression: the old guess list locked onto ONE key, so this previewed
        # as just "张三" — losing the group and that a message was attached.
        result = build_tool_preview(
            "yb_send_dm", {"group_code": "G9", "name": "张三", "message": "hi"}
        )
        assert "group_code=G9" in result
        assert "name=张三" in result

    def test_bespoke_branches_are_unchanged(self):
        assert build_tool_preview("terminal", {"command": "ls -la"}) == "ls -la"
        assert build_tool_preview("web_search", {"query": "weather"}) == "weather"
        assert build_tool_preview("read_file", {"path": "/tmp/x.py"}) == "x.py"

    def test_empty_args_still_return_none(self):
        # The one contract preserved: no args means no preview, so callers
        # using `or function_name` keep working.
        assert build_tool_preview("computer_use", {}) is None
        assert build_tool_preview("anything", None) is None
