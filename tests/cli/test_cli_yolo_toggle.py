...
``os.environ["ARGUS_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``hermes --yolo`` (process-start env), ``ARGUS_YOLO_MODE=1``,
and ``argus config set approvals.mode off`` actually bypassed.
...
class TestToggleYoloIsSessionScoped:
    ...
    def test_toggle_yolo_does_not_mutate_env_var(self):
        """Toggling /yolo must not write ``ARGUS_YOLO_MODE`` — that path is
        ...
        assert os.environ.get("ARGUS_YOLO_MODE") is None
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``hermes --yolo`` sets ``ARGUS_YOLO_MODE`` before tool imports, so
