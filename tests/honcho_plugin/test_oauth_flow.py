...
def test_config_path_rides_the_authorize_link(fake_as):
    endpoints = oauth_flow.resolve_endpoints()
    url, _ = oauth_flow.begin_authorization(endpoints, config_path="~/.argus/honcho.json")
    q = parse_qs(urlparse(url).query)
    assert q["config_path"][0] == "~/.argus/honcho.json"
...
def test_display_config_path_never_leaks_absolute_path():
    ...
    under_home = Path.home() / ".argus" / "profiles" / "work" / "honcho.json"
    assert oauth_flow._display_config_path(under_home) == "~/.argus/profiles/work/honcho.json"
