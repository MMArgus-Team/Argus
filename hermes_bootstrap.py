...
def harden_import_path(src_root: str | None = None) -> None:
    ...
    """
    root = src_root or os.environ.get("ARGUS_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``ARGUS_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("ARGUS_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
