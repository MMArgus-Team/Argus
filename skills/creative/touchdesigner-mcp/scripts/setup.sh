...
ARGUS_HOME_DIR="${ARGUS_HOME:-$HOME/.argus}"
ARGUS_CFG="${ARGUS_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
if [[ ! -f "$ARGUS_CFG" ]]; then
    echo -e " ${FAIL} Hermes config not found at ${ARGUS_CFG}"
    manual_steps+=("Create ${ARGUS_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ARGUS_CFG" 2>/dev/null; then
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ARGUS_CFG} manually"); }
