...
# Non-root user for runtime; UID can be overridden via ARGUS_UID at runtime
...
# tree), NOT into $ARGUS_HOME. $ARGUS_HOME (/opt/data) is a shared data
...
# `s6-setuidgid hermes` in its run script. If ARGUS_UID is unset, services
...
#
# Fix: write the commit SHA passed via the ARGUS_GIT_SHA build-arg to
# /opt/hermes/.argus_build_sha at build time, and have
...
ARG ARGUS_GIT_SHA=
RUN if [ -n "${ARGUS_GIT_SHA}" ]; then \
        printf '%s\n' "${ARGUS_GIT_SHA}" > /opt/hermes/.argus_build_sha; \
    fi
...
# slots from $ARGUS_HOME/profiles/<name>/ after a container restart
...
# ---------- Runtime ----------
ENV ARGUS_WEB_DIST=/opt/hermes/hermes_cli/web_dist
...
ENV ARGUS_TUI_DIR=/opt/hermes/ui-tui
ENV ARGUS_HOME=/opt/data
ENV ARGUS_WRITE_SAFE_ROOT=/opt/data
ENV ARGUS_DISABLE_LAZY_INSTALLS=1
...
ENV ARGUS_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> hermes ...` they default to root, and any file the
# command writes under $ARGUS_HOME (auth.json, .env, config.yaml) ends
...
# the opt-out env var (ARGUS_DOCKER_EXEC_AS_ROOT=1).
